...
class RefPath:
    def __init__(self, center_line: np.ndarray, width_left: Union[np.ndarray, float] = 0.5,
                    width_right: Union[np.ndarray, float] = 0.5,
                    speed_limt: Union[np.ndarray, float] = 1, loop: Optional[bool] = False) -> None:
        '''
    ...
    def interp_s(self, s: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def get_closest_pts(self, points: np.ndarray, eps: Optional[float] = 1e-5) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        if points.shape[1] == 1:
            s = np.array([s])
        closest_pt, slope = self.interp_s(s)
    ...
    def local2global(self, local_states: np.ndarray, return_slope=False) -> np.ndarray:
        ...
        global_states, slope = self.interp_s(progress)
        if num_pts == 1:
